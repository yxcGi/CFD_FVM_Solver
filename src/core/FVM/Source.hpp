...
namespace fvm {
                ...
                const CellField<Tp> &S_c)
    {
        ...
        if (!S_p.isValid()) {
            std::cerr << "Source() Error: S_p is not valid." << std::endl;
        ...
        // 判断输入参数是否为同一网格
        if (mesh != S_p.getMesh()) {
            std::cerr << "Source() Error: matrix and S_p are not constructed by the same mesh." << std::endl;
            throw std::invalid_argument("matrix and S_p are not constructed by the same mesh.");
        }
        if (mesh != S_c.getMesh()) {
        ...
        for (ULL cellId = 0; cellId < mesh->getCellNumber(); ++cellId) {
            ...
            matrix(cellId, cellId) -= cells[cellId].getVolume() * S_p[cellId];
        }
    }
